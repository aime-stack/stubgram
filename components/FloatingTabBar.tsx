...
export default function FloatingTabBar({
  tabs,
  containerWidth = screenWidth * 0.85, // 85% of screen width for 4 tabs
  borderRadius = 30,
  bottomMargin
}: FloatingTabBarProps) {
  ...
  const activeTabIndex = React.useMemo(() => {
    ...
    tabs.forEach((tab, index) => {
      ...
      // Ensure tab.route exists and is a string before processing
      ...
      else if (tab.route && typeof tab.route === 'string' && tabRoute.includes('/(tabs)/') && pathname.includes(tabRoute.split('/(tabs)/')[1])) {
        score = 40;
      }
  ...
  const handleTabPress = (route: Href) => {
    ...
    if (router && router.push) {
      ...
      console.log('Router not available yet, skipping navigation to:', route);
    }
  };
